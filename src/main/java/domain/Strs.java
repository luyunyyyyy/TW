package domain;

public class Strs {
    public static final String NORMAL = "Success: the booking is accepted!";
    public static final String INVALID = "Error: the booking is invalid!";
    public static final String CONFLICTS = "Error: the booking conflicts with existing bookings!";
    public static final String NOTEXISTS = "Error: the booking being cancelled does not exist!";
}
